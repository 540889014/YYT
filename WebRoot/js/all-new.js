	...
	var zzs={
		...
		ismsie:/msie/i.test(navigator.userAgent),
		html5:function(){
			...
			if(this.isff || this.ischrome || this.ismsie){
				return false;
			}else{
				return true;
			}
	...
	ScrollBar.prototype={
		...
		sixthAnimation:function(){
			...
			var attr=[{left:zzs.isapple() ? -70 : 0,opacity:1},{right:zzs.isapple() ? -70 : 0,opacity:1},{left:0,opacity:1},{right:0,opacity:1},{opacity:1},{width:230},{height:52},{bottom:0,opacity:1}];
	...
	Scroll2.prototype={
		...
		event:function(){
			...
			var doMove=function(){
				...
				me.ul.animate({left:-me.li.width()},"slow","easeInOutBack",function(){
					var ul=me.id.find("ul");
					var li=me.id.find("li");
					ul.append(li.eq(0));
					ul.css("left",0);
					me.done=!me.done;
				});
			...
			var autoPlay=function(){
				autoPlay.timer=setInterval(doMove,3500);
			}
			
			
			var together=function(){
				autoPlay();
				me.id.hover(function(){
					clearInterval(autoPlay.timer);
				},function(){
					autoPlay();
				});
			}
			if(this.auto){together();}
		...
		event2:function(){
			...
			this.l.bind("click",function(){
				...
				_this.ul.animate({left:-_this.li.width()*i},"slow","easeInOutBack",function(){
					_this.done=!_this.done;
					if(_this.ul.position().left<=-mini){
						_this.l.hide();
					}
				});
			});
